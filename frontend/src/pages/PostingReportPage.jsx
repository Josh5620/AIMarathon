    ...
    <>
      ...
      <article className="max-w-[960px] px-[48px] py-[40px]">
        ...
        <header className="mb-[40px]">
          ...
          <h2 className="font-brand text-[1.25rem] font-bold text-on-surface tracking-[-0.01em] mb-[4px]">
        ...
        <section className="grid grid-cols-3 gap-px bg-outline-variant border border-outline-variant mb-[48px]">
          <div className="bg-surface-container-lowest p-card-padding text-center">
            <div className="font-brand text-[2.4rem] font-extrabold text-on-surface leading-none">{applicants.length}</div>
            <div className="text-[0.78rem] text-on-surface-variant mt-xs">Total applicants</div>
          </div>
          <div className="bg-surface-container-lowest p-card-padding text-center">
            <div className="font-brand text-[2.4rem] font-extrabold text-on-surface leading-none">{interested.length}</div>
            <div className="text-[0.78rem] text-on-surface-variant mt-xs">Interested</div>
          </div>
          <div className="bg-surface-container-lowest p-card-padding text-center">
            <div className="font-brand text-[2.4rem] font-extrabold text-on-surface leading-none">{avgMatch != null ? `${avgMatch}%` : '—'}</div>
        ...
        {posting.description && (
          <section className="mb-[40px]">
            <div className="flex justify-between items-baseline mb-[20px] pb-[12px] border-b-2 border-on-surface">
              <h3 className="font-brand text-[1.3rem] font-bold text-on-surface">Job Description</h3>
            </div>
        ...
        {posting.requirements && (
          <section className="mb-[40px]">
            <div className="flex justify-between items-baseline mb-[20px] pb-[12px] border-b-2 border-on-surface">
              <h3 className="font-brand text-[1.3rem] font-bold text-on-surface">Requirements</h3>
            </div>
            <p className="whitespace-pre-wrap leading-relaxed text-on-surface text-[0.875rem]">
              {posting.requirements}
            </p>
        ...
        <section>
          <div className="flex justify-between items-baseline mb-[20px] pb-[12px] border-b-2 border-on-surface">
            <h3 className="font-brand text-[1.3rem] font-bold text-on-surface">Ranked Applicants</h3>
              ...
              <div key={app.application_id} className={`grid grid-cols-[auto_1fr_auto] items-start gap-lg py-[24px] px-md border-b break-inside-avoid ${
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-sm flex-wrap mb-[4px]">
                    <span className="font-brand text-[1.1rem] font-bold text-on-surface tracking-[-0.01em]">{app.name || 'Unknown'}</span>
                    {app.is_interested && (
                ...
                <div className="flex-shrink-0 text-right pt-[2px]">
                  <div className={`font-brand text-[1.5rem] font-extrabold leading-none ${scoreColor(app.rank_score)}`}>
